...
// ... rest of your script stays same ...
    ...
    if (window.YT && window.YT.Player) {
     ...
     function changeDecade(decade, index = 0) {
  ...
  // Use the passed index (or 0 if not provided)
  currentIndex = index;
  ...
  // Pass both decade and currentIndex so it continues from the same position
  changeDecade(selectedDecade, currentIndex);
  if (!ytPlayer) return; ytPlayer.pauseVideo(); setTimeout(() => ytPlayer.playVideo(), 1200);
}
  ...
  // First autoplay → force pause (keeps your original behavior)
